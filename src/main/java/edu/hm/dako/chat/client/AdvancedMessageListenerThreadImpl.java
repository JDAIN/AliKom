...
/**
 * Thread wartet auf ankommende Nachrichten vom Server und bearbeitet diese. Nach der Bearbeitung
 * einer Nachricht bestätigt er diese.
 * 
...
public class AdvancedMessageListenerThreadImpl extends AbstractMessageListenerThread {
  ...
  protected void chatMessageResponseAction(ChatPDU receivedPdu) {
    ...
    log.debug(Thread.currentThread().getName()
        ...
        + receivedPdu.getServerTime() + " ns = " + receivedPdu.getServerTime() / 1000000 + " ms");
    ...
    if (receivedPdu.getSequenceNumber() == sharedClientData.messageCounter.get()) {
      ...
      log.debug(
          "Sequenznummer der Chat-Response-PDU " + receivedPdu.getUserName() + " passt nicht: "
  ...
  /**
   * Aktion zur Behandlung ankommender Message-Confirms. Wird im chatMessageEvent aufgerufen.
   * 
   * @param receivedPdu
   *          Ankommende PDU
   */
  protected void chatMessageConfirmAction(ChatPDU receivedPdu) {
    ...
    try {
      ...
      log.debug("Client " + receivedPdu.getUserName()
          + " sendet Chat-Confirm-Event-PDU zur Nachricht von " + receivedPdu.getEventUserName());
  ...
  protected void loginConfirmAction(ChatPDU receivedPdu) {
    ...
    try {
      ...
      log.debug("Client " + sharedClientData.userName
          + " sendet Login-Confirm-Event-PDU zum Login von " + receivedPdu.getEventUserName());

    } catch (Exception e) {
      log.debug(
          "Senden der Confim-Log-In-Nachricht von " + receivedPdu.getUserName() + "nicht moeglich");
      log.debug("Exception Message: " + e.getMessage());
    }
  }
  ...
  protected void logoutConfirmAction(ChatPDU receivedPdu) {
    ...
    try {
      ...
      log.debug("Client " + sharedClientData.userName
          + " sendet Log-Out-Confirm-Event-PDU zum Log Out von " + receivedPdu.getEventUserName());

    } catch (Exception e) {
      log.debug("Senden der Confim-Log-Out-Nachricht von " + receivedPdu.getUserName()
          + "nicht moeglich");
      log.debug("Exception Message: " + e.getMessage());
    }
  }
  ...
  protected void loginResponseAction(ChatPDU receivedPdu) {
    ...
    if (receivedPdu.getErrorCode() == ChatPDU.LOGIN_ERROR) {
      ...
      try {
        ...
        log.error("verbindung konnte nicht geschlossen werden: " + e);
      }
