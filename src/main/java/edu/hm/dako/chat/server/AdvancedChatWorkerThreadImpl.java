...
public class AdvancedChatWorkerThreadImpl extends AbstractWorkerThread {
	...
	protected void logoutRequestAction(ChatPDU receivedPdu) {
		...
		if (!clients.existsClient(userName)) {
			...
			//// // gesendet
			//// // wird, da sich sonst ein Client abmeldet, bevor er seinen
			// letzten
			//// // Event
			//// // empfangen hat. das funktioniert nicht bei einer grossen
			// Anzahl an
			//// // Clients (kalkulierte Events stimmen dann nicht mit
			// tatsaechlich
			//// // empfangenen Events ueberein.
			//// // In der Advanced-Variante wird noch ein Confirm gesendet, das
			// ist
			//// // sicherer.
			//// try {
			//// Thread.sleep(1000);
			...
			//// log.debug("Laenge der Clientliste beim Vormerken zum Loeschen
			// von " + receivedPdu.getUserName() + ": "
			//// + clients.size());
		}
	}
	...
	private void loginConfirmAction(ChatPDU receivedPdu) {
		...
		// wenn warteliste leer ist LoginResponse an Event initiator senden
		if (clients.getWaitListSize(receivedPdu.getEventUserName()) == 0) { // neu
			// Login Response senden
			ChatPDU responsePdu = ChatPDU.createLoginResponsePdu(receivedPdu.getEventUserName(), receivedPdu); // das
																												// geändert
			...
			// log.debug("\n \n loginconfrimaction vor change \n \n"
			// +clients.getClientStatus(userName) + "\n"+ "\n" +"\n" +
			// receivedPdu.toString() + "\n \n \n" + responsePdu.toString() +
			// "\n \n");
			//
			// log.debug("\n \n loginconfrimaction nach change \n \n"
			// +clients.getClientStatus(userName) + "\n \n");
		}
	}
	...
	protected void chatMessageConfirmAction(ChatPDU receivedPdu) {
		...
		log.debug("ChatMessageConfirm empfangen von Client" + receivedPdu.getUserName() + " zu Nachricht von "
