...
/**
 * Worker-Thread zur serverseitigen Bedienung einer Session mit einem Client. Jedem Chat-Client wird
 * serverseitig ein Worker-Thread zugeordnet.
 * 
 * @author Peter Mandl
 *
 */
  ...
  protected void loginRequestAction(ChatPDU receivedPdu) {
    ...
    if (!clients.existsClient(receivedPdu.getUserName())) {
      ...
      try {
        ...
        log.debug(
            "Senden einer Login-Response-PDU an " + receivedPdu.getUserName() + " nicth moeglich");
